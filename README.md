...
```bash
npm install react-native-image-slideshow --save
```


```bash
npm install --save prop-types // it require prop-types
```
...
```javascript
export default class ImageSliderzTest extends Component {
  constructor(props) {
    super(props);
...
| --- | :---: | :---: | :---: | --- |
| `dataSource` | bool | required | - | imageSliderz data |
| `height` | number | optional | 200 | container height |
| `position` | number | optional | - | set position imageSliderz |
...
## Credits
[react-native-slideshow](https://github.com/haqiqiw/react-native-slideshow)
